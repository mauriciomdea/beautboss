...
RSpec.describe "Newsfeed API v1", type: :request do
  ...
  describe "POST /api/v1/newsfeed" do
    ...
    it "returns newsfeed for authorized user" do
      ...
      for i in 1..3
        FactoryGirl.create :post_public, user: john, service: "Post #{i} from #{john.name}", created_at: i.seconds.ago
        FactoryGirl.create :post_public, user: jane, service: "Post #{i} from #{jane.name}", created_at: i.seconds.ago
        FactoryGirl.create :post_public, service: "Post #{i} from Someone Else", created_at: i.seconds.ago
      end
      ...
      expect(body["posts"][0]["service"]).to eq "Post 1 from John Doe"
